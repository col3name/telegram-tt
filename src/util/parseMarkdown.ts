import {
  ApiFormattedText,
  ApiMessageEntity, ApiMessageEntityDefault,
  ApiMessageEntityPre,
  ApiMessageEntityTypes,
} from '../api/types';
import { ENTITY_CLASS_BY_NODE_NAME, fixImageContent } from './parseHtmlAsFormattedText';

const urlRegexPattern = /https?:\/\/[^\s/$.?#].[^\s]*/gi;

function extractUrlsWithDetails(inputString: string): ApiMessageEntityDefault[] {
  // Regular expression to match URLs

  // Array to store results
  const urls = [];
  // Use a loop to find all matches and their positions
  let match;
  // eslint-disable-next-line no-null/no-null,no-cond-assign
  while ((match = urlRegexPattern.exec(inputString)) !== null) {
    const url = match[0];
    const offset = match.index;
    const length = url.length;

    urls.push({
      type: ApiMessageEntityTypes.Url, url, offset, length,
    } as ApiMessageEntityDefault);
  }

  return urls;
}
function parseEnclosedContent(
  value: string,
  startIndex: number,
...
function parseEmoji(value: string, startIndex: number) {
  let i = startIndex + 1;
...
function parseMention(value: string, startIndex: number) {
  let i = startIndex + 1;
...
function parseHashtag(value: string, startIndex: number) {
  let i = startIndex + 1;
...
function parseCommand(value: string, startIndex: number) {
  let i = startIndex + 1;
  ...
  if (i === commandStart) {
    return undefined; // No valid command found
  }
...
function parseMarkdownAsAst(value: string, offset: number = 0) {
  const tokens = [];
  let i = 0;
  // let urlIndex = 0;
  ...
  while (i < value.length) {
    const char = value[i];
    // const match = urlRegexPattern.exec(value);
    // if (match !== null) {
    //   if (urlIndex < match.length) {
    //     const url = match[urlIndex];
    //     const offset2 = offset + i + 1 + match.index;
    //     urlIndex++;
    //     tokens.push({ type: 'simpleLink', value: url, offset: offset2 + i });
    //     i = offset2 + i + url.length;
    //   }
    // }
    ...
    if (char === '*') {
      ...
      if (value[i + 1] === '`' && value[i + 2] === '`') {
        ...
        const [content, endIndex] = parseEnclosedContent(value, i, '```', '```');
        const language = content.length > 1 ? content.split('\n')[0] : undefined;
        tokens.push({
          type: 'pre',
          value: language ? content.substring(language?.length) : content,
          offset: offset + i + 2,
          language,
        });
        i = endIndex;
...
function parseCashtag(value: string, startIndex: number) {
  let i = startIndex + 1;
  ...
  if (i === symbolStart) {
    return undefined; // No valid cashtag found
  }
...
function parsePhoneNumber(value: string, startIndex: number) {
  let i = startIndex + 1;
  ...
  if (i === numberStart) {
    return undefined; // No valid phone number found
  }
...
function parseEmail(value: string, startIndex: number) {
  let i = startIndex;
...
function isMentionStart(value: string, startIndex: number) {
  return value[startIndex] === '@' && /^[a-zA-Z0-9_]/.test(value[startIndex + 1]);
}

function parseBlockquote(value: string, startIndex: number, offset: number,) {
  let i = startIndex;
  const children = [];
...
function parseLinkOrCustomEmoji(value: string, startIndex: number) {
  let i = startIndex + 1;
  ...
  if (match) {
    ...
    return {
      type: 'mention_user', id: userId, value: text.trim(), offset: startIndex + i, endIndex: i + 1,
    };
  }

  return {
    type: 'link', url, value: text, endIndex: i + 1, offset: startIndex + i,
  };
}

function parseBankCard(value: string, startIndex: number) {
  let i = startIndex;
  let rawCardNumber = '';

  while (i < value.length && /[0-9\s/-]/.test(value[i])) {
    if (/[0-9]/.test(value[i])) {
      rawCardNumber += value[i];
    }
    i++;
  }
...
export function convertMarkdownIntoApiFormattedText(html: string): ApiFormattedText {
  ...
  function astToApiFormatedText(nodes) {
    if (Array.isArray(nodes)) {
      nodes
        .forEach((node) => {
          const length1 = node.value?.length;
          if (isTextNode(node) && length1 > 0) {
            switch (node.type) {
              ...
              case 'pre': {
                ...
                const value: ApiMessageEntityPre = {
                  type: ApiMessageEntityTypes.Pre,
                  offset: Math.max(0, text.length - length1),
                  length: length1,
                };
                if (node.language) {
                  value.language = node.language;
                }
                entities.push(value);
                break;
              }
          ...
          if (node.children) {
            astToApiFormatedText(node.children);
          }
          const length = getLength(node);
          switch (node.type) {
            case 'simpleLink': {
              const offset = Math.max(0, text.length - length1);
              if (offset !== length1) {
                text += node.value;
                entities.push({
...
export function parseHtmlAsApiFormattedText(html: string, withMarkdownLinks: boolean = false) {
  const fragment = document.createElement('div');
  fragment.innerHTML = cleanHtml(html);
  fixImageContent(fragment);
  const text = fragment.innerText.trim().replace(/\u200b+/g, '');
  const trimShift = fragment.innerText.indexOf(text[0]);
  let textIndex = -trimShift;
  let recursionDeepness = 0;
  const entitiesList: ApiMessageEntity[] = [];
  ...
  return {
    text,
    entities: entitiesList.length ? entitiesList : undefined,
  };
}
